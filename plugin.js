...
function pickOne() {
    ...
    if (!easy && !medium && !hard) {
        ...
        $("#pickModalContent").text("Please choose one at least.");
        $("#b_yes").attr('disabled', true);
        $("#b_repick").attr('disabled', true);
    } else {
        var select = ""
        ...
        if (problem_text == "") {
            ...
            $("#pickModalContent").text("Picked a cat :P");
            $("#b_yes").attr('disabled', true);
            $("#b_repick").attr('disabled', true);
        } else {
            $("#pickModalContent").html("[" + problem_text + "]");
            $("#b_yes").attr('disabled', false);
            $("#b_repick").attr('disabled', false);
            $("#b_yes").bind("click", function () { window.location.href = problem.find("a").attr("href"); });
            //$("#b_ensurePick").href = problem.find("a").attr("href");
            //$("#b_message").text("");
            //confirm("Pick [" + problem_text + "] ?")
